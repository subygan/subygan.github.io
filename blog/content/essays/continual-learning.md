...
description: and why a small agent that learns will beat a bigger model
image: danaides.jpg
imageSocial: danaides-og.jpg
imageFull: true
...
worth noting he doesn't think this is unsolvable, just unsolved. he even flips it into the scary direction, "an AI that is capable of online learning might functionally become a superintelligence quite rapidly without any further algorithmic progress." his own bet on when we get there for real white collar work is 2032.

Satya Nadella came at it from the other side of the table, and his version is the one that actually reframed it for me. his point isn't that the model can't learn. it's that the model _is_ learning, from you, constantly, and the learning is the asset.
...
and Ilya, who left OpenAI and bet a whole company on the idea that scaling alone doesn't get you there. he keeps saying the quiet part in the plainest possible way.
...
[^karpathy]: Andrej Karpathy on the missing consolidation step (working memory vs weights, the "cognitive core"). this is paraphrase of his known position, not a verbatim quote. TODO: drop the exact source (i think it's the Dwarkesh podcast, Oct 2025) and i'll swap in his words.
...
[^letta]: Letta / MemGPT — the model-as-OS memory hierarchy ([memgpt paper](https://arxiv.org/abs/2310.08560)).
[^seal]: Zweiger, Pari, Guo, Akyürek, Kim, Agrawal, ["Self-Adapting Language Models"](https://arxiv.org/abs/2506.10943) (MIT, 2025). numbers above (SQuAD 32.7→47.0, beats GPT-4.1 synthetic at 46.3, ARC 0→72.5) and the forgetting quote are verbatim from the paper.
[^lora]: Thinking Machines, ["LoRA Without Regret"](https://thinkingmachines.ai/blog/lora/). quotes verbatim.
[^sglang]: [batch lora in SGLang](/tech/tinkering/sglang-batch-lora) — the serving side, still a stub, sorry.
[^ttt]: test time training / test time RL. TODO: pick the representative paper you like best (the SEAL ARC result above is itself built on a TTT baseline).
[^nested]: Google Research, ["Introducing Nested Learning"](https://research.google/blog/introducing-nested-learning-a-new-ml-paradigm-for-continual-learning/) and the HOPE architecture (2025). quotes verbatim from the blog. (double check the exact URL slug, i reconstructed it.)
[^guardrails]: [llm guardrails](/tech/ML/llm_guardrails) — the REST-API-limits-innovation complaint, 2024, still unfixed.
[^rl]: [reinforcement learning in pictures](/tech/ML/rl_in_pictures) — reward hacking, the model that draws a four instead of four animals.
